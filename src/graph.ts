...
export function addNeighbors(graph: Graph, nodeId: string, neighborIds: string[], defaultWeight: number = 1): Graph {
  let newGraph = { ...graph };
  for (const neighborId of neighborIds) {
    const newEdge: Edge = { from: nodeId, to: neighborId, weight: defaultWeight };
